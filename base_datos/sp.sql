...
BEGIN
	select (u.nombre_usuario + ''+ u.apellido_paterno + ''+ u.apellido_materno) as nombre_usuario, c.id_carrera, c.nombre_carrera, s.fecha_creacion, e.nombre_estatus
	from solicitud
...
INSERT INTO solicitud (interesado, coordinador,solicitud,motivo_academico, motivo_personal, motivo_otro, id_estatus)VALUES (numero_interesado,numero_coordinador, solicitud, motivo__academico, motivo__personal, motivo__otro,1);	

END $$
----------------------------------------------------------------------------------------------------------------
DELIMITER $$
CREATE PROCEDURE consulta_por_carrera (
	IN id__carrera tinyint
)
BEGIN
	select (u.nombre_usuario + ''+ u.apellido_paterno + ''+ u.apellido_materno) as nombre_usuario, c.id_carrera, c.nombre_carrera, s.fecha_creacion, e.nombre_estatus
	from solicitud
...
DELIMITER $$
CREATE PROCEDURE consulta_soliicitud_por_id_solicitud (
...
BEGIN
	...
	where c.id_solicitud= id__solicitud;
END $$
----------------------------------------------------------------------------------------------------------