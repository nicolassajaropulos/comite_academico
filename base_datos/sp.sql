...
BEGIN
	select (u.nombre_usuario + ''+ u.apellido_paterno + ''+ u.apellido_materno) as nombre_usuario, c.id_carrera, c.nombre_carrera, s.fecha_creacion, s.ultima_modificacion, e.nombre_estatus
	from solicitud
	INNER JOIN usuario u
	ON s.interesado  = u.numero_control
	INNER JOIN usuario_carrera uc
	...
	INNER JOIN estatus e
	ON s.id_estatus = e.id_estatus
	where s.numero_control = numero__control;
END $$